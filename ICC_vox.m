...
% Last Modified by GUIDE v2.5 08-Feb-2017 13:31:20
...
save_nii(target_img,target_img.fileprefix); 
     ...
     for ind_x = 1:x
        for ind_y = 1:y
           for ind_z = 1:z
                for ind_run = 1:runs
                    ev=sprintf('data_%d = zeros(nr_subj,2)',ind_run);
                    eval(ev)
                    for ind_subj = 1:nr_subj
                    ...
                    estr = sprintf('data_%d(:,1)=img%d_voxel_split1;',ind_run);
                    eval(estr);
                    estr2 = sprintf('data_%d(:,2)=img%d_voxel_split2;',ind_run);
                    eval(estr2);                    
                end;
                    %ICC
                    for ind_run = 1:runs
