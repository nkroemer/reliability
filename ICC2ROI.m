...
% Last Modified by GUIDE v2.5 17-Mar-2017 12:25:49
...
temp_img = sprintf('%s\\%s\\%s\\%s',stats_path,vp{1},stats_filled,con); % Beispielbild Kontrast
...
if cons == 1
    ...
    for i = 1:nr_clusters
        ...
        for k = 1:length(lab_neu)
            count = 0;
            for l = 1:length(labels_voxels)
                if strcmp(lab_neu{k,1},labels_voxels{l,1}) == 1
                    count = count+1;
                end;
            end;
            lab_neu{k,2} = count;
