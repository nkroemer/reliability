...
% Last Modified by GUIDE v2.5 02-Jun-2017 14:07:28
...
if use_roi == 1
    ...
    roi_compl = sprintf('%s%s%s%s',roi_dir,f,f,roi_name);
    disp('...reslicing ROI...');
    stats_filled = sprintf(stats,1);
    if two_cons == 1
        temp = sprintf('%s%s%s%s%s%s%s%s%s%s,1',path,f,f,vp{1},f,f,stats_filled,f,f,con1);
    else
        temp = sprintf('%s%s%s%s%s%s%s%s%s%s,1',path,f,f,vp{1},f,f,stats_filled,f,f,con);
    end;
    ...
    r_roi = sprintf('%s%s%sr%s',roi_dir,f,f,roi_name);
...
if two_cons == 0 && split == 0
    for ind_run = 1:runs
        ...
        file = sprintf('%s%s%s4D_%d.nii',results_dir,f,f,ind_run);
    ...
    if nr_para > 0
        for ind_para = 1:nr_para
            for ind_run = 1:runs
                ...
                file = sprintf('%s%s%s4D_par%d_%d.nii',results_dir,f,f,ind_para,ind_run);
                eval(sprintf('FourD%d = file;',ind_run));
                eval(sprintf('FourD%d = load_nii(FourD%d);',ind_run,ind_run));
                eval(sprintf('FourD%d_par%d = FourD%d.img;',ind_run,ind_para,ind_run));
                ...
                for ind_x = 1:x
                    for ind_y = 1:y
                        for ind_z = 1:z 
                            eval(sprintf('mean_FourD%d(ind_x,ind_y,ind_z) = mean(FourD%d_par%d(ind_x,ind_y,ind_z,:));',ind_run,ind_run,ind_para));
                        end;
                    end;
                end;
                eval(sprintf('mean_FourD%d_par%d = mean_FourD%d(~isnan(mean_FourD%d));',ind_run,ind_run,ind_run,ind_para));
            end;
        end;
    end;
elseif two_cons == 1
     for ind_run = 1:runs
        ...
        file1 = sprintf('%s%s%s4D_%s_%d.nii',results_dir,f,f,con1,ind_run);
        ...
        file2 = sprintf('%s%s%s4D_%s_%d.nii',results_dir,f,f,con2,ind_run);
    ...
    if nr_para > 0
        for ind_para = 1:nr_para
            for ind_run = 1:runs
                ...
                file1 = sprintf('%s%s%s4D_%s_par%d_%d.nii',results_dir,f,f,con1,ind_para,ind_run);
                eval(sprintf('FourD%d = file1;',ind_run));
                eval(sprintf('FourD%d = load_nii(FourD%d);',ind_run,ind_run));
                ...
                file2 = sprintf('%s%s%s4D_%s_par%d_%d.nii',results_dir,f,f,con2,ind_para,ind_run);
                eval(sprintf('FourD%d = file2;',ind_run));
                eval(sprintf('FourD%d = load_nii(FourD%d);',ind_run,ind_run));
...
for ind_run = 1:runs
        ...
        file1 = sprintf('%s%s%s4D_split1_%d.nii',results_dir,f,f,ind_run);
        eval(sprintf('FourD%d = file1;',ind_run));
        eval(sprintf('FourD%d = load_nii(FourD%d);',ind_run,ind_run));
        ...
        file2 = sprintf('%s%s%s4D_split2_%d.nii',results_dir,f,f,ind_run);
        eval(sprintf('FourD%d = file2;',ind_run));
        eval(sprintf('FourD%d = load_nii(FourD%d);',ind_run,ind_run));
...
if nr_para > 0 
    for ind_para = 1:nr_para
        for ind_run = 1:runs
                ...
                file1 = sprintf('%s%s%s4D_split1_par%d_%d.nii',results_dir,f,f,ind_para,ind_run);
                eval(sprintf('FourD%d = file1;',ind_run));
                eval(sprintf('FourD%d = load_nii(FourD%d);',ind_run,ind_run));
                ...
                file2 = sprintf('%s%s%s4D_split2_par%d_%d.nii',results_dir,f,f,ind_run,ind_para);
                eval(sprintf('FourD%d = file2;',ind_run));
                eval(sprintf('FourD%d = load_nii(FourD%d);',ind_run,ind_run));
