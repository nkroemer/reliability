...
% Last Modified by GUIDE v2.5 26-Jul-2017 09:53:07
...
% --- Executes on button press in use_corr.
function use_corr_Callback(hObject, eventdata, handles)
% hObject    handle to use_corr (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of use_corr


% --- Executes on button press in use_icc.
function use_icc_Callback(hObject, eventdata, handles)
% hObject    handle to use_icc (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of use_icc
...
cluster_name = sprintf('%s_cluster_%d_%0.2f.mat',prefix,k,r_thres);
save(cluster_name,'C');
target_img = temp_img;
...
for i = 1:nr
    if i<nr
        condition = sprintf('%s ICC_map%d(ind_x,ind_y,ind_z) >= r_thres &&',condition,i);
    elseif i == nr
        condition = sprintf('%s ICC_map%d(ind_x,ind_y,ind_z) >= r_thres',condition,i);
    end;
end;
