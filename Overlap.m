...
% Last Modified by GUIDE v2.5 02-Jun-2017 14:33:34
...
if split == 0 && two_cons == 0
    for i = 1:length(vp)
        ...
        for j = 1:runs
            ...
            SPM_path = sprintf('%s%s%s%s%s%s%s',stats_path,f,f,vp{i},f,f,stats_dir_filled);
            cd(box_path);
...
elseif two_cons == 1
 ...
 %create results table with names
    ...
    for k = 1:runs
            ...
            if eval(sprintf('nr_sig%d_split1',k)) > eval(sprintf('nr_sig%d_split2',k))
                count = 0;
                    for l = 1:eval(sprintf('nr_sig%d_split1',k))
                        for m = 1:eval(sprintf('nr_sig%d_split2',k))
                            comp = isequal(eval(sprintf('co_sig%d_split1(:,l)',k)),eval(sprintf('co_sig%d_split2(:,m)',k)));
                            count = count + comp;
                        end;
                    end;
            else
                count = 0;
                    for l = 1:eval(sprintf('nr_sig%d_split2',k))
                        for m = 1:eval(sprintf('nr_sig%d_split1',k))
                            comp = isequal(eval(sprintf('co_sig%d_split1(:,m)',k)),eval(sprintf('co_sig%d_split2(:,l)',k)));
                            count = count + comp;
                        end;
                    end;
            end;
